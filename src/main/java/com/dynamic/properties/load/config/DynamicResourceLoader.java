package com.dynamic.properties.load.config;

import com.dynamic.properties.load.exception.ResourceException;
			...
			if(!properties.containsKey(location))
			{
				try
				{
					...
					if(!file.exists())
					{
						...
						if (resource == null) {
							throw new ResourceException("Resource not found: " + location);
						}
						file = new File(resource.getPath());
					}
					byte[] content = this.getBytesFromUrl(resource);
					if (file != null) {
						properties.put(location, new PropertyParts<>(resource, content, file.lastModified()));
					} else {
						properties.put(location, new PropertyParts<>(resource, content, null));
					}
